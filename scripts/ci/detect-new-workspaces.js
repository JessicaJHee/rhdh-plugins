...
async function ensureRenovateExtends(presetRef) {
  ...
  const pattern =
    /("description":\s*"all RHDH Plugins workspaces"[\s\S]*?"extends":\s*\[[\s\S]*?)(\n\s*\])/;
  const match = content.match(pattern);

  if (!match) {
    throw new Error(
      'Could not find "all RHDH Plugins workspaces" rule with extends array in .github/renovate.json',
    );
  }
...
function parseCodeownersOwnersFor(workspaceName, codeownersContent) {
  const pattern = new RegExp(`^/workspaces/${workspaceName}\\s+(.+)$`, 'm');
  const match = codeownersContent.match(pattern);
  if (!match) return [];
  ...
  case '--apply': {
    ...
    if (process.env.GITHUB_OUTPUT) {
      ...
      fs.appendFileSync(
        ...
        `reviewers=${reviewers.reviewers.join(',')}\n`,
      );
      fs.appendFileSync(
        process.env.GITHUB_OUTPUT,
        `team_reviewers=${reviewers.team_reviewers.join(',')}\n`,
      );
    }
